    ...
    if (data.length > 0) {
        data.forEach((item, index) => {
            ...
            row.innerHTML = `
                ...
                <td>${item.wmName || ''}</td>
                <td>${item.email || ''}</td>
                <td><button onclick="showQuotationDetails('${item._id}')">View Quotation</button></td>
            `;
        });
    } else {
        const row = table.insertRow();
...
async function confirmApproval() {
    ...
    try {
        const response = await fetch(`/api/approve-or-reject`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: registrationId, action: 'approve' })
        });
...
async function confirmRejection() {
    ...
    try {
        const response = await fetch(`/api/approve-or-reject`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: registrationId, action: 'reject', reason })
        });
